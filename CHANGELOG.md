...
- **"Due today" and "Overdue" quick-filter pill buttons on Assignments page** — orange "Due today · N" and red "Overdue · N" pill buttons appear conditionally in the filter section when there are matching pending assignments. Clicking toggles the search term, wiring directly into the existing `matchesAssignmentSearch` natural-language filter that already handles "today" / "overdue" keywords. No new filter logic needed.
  **Why:** Students frequently need to triage overdue work or see exactly what's due today; previously they had to type the search terms manually or scroll through the full list.
